...
bool Login::checkPass(QString login,QString pass)
{
  ...
  if(login.length()>3)
  {
     QByteArray ar = login.toLocal8Bit();
     quint32 hash = maPrime2dHash (ar);
     if(pass==QString::number(hash,16) )
     {
         stat =true;
     }
  }
...
uint32_t Login::maPrime2dHash (QByteArray array)
{
    static constexpr unsigned char sTable[256] =
    {
